...
| `REASONING_EFFORT` | 推理强度（low/medium/high），自动注入请求。注意：Gemini 3 Pro 仅支持 low/high | 留空不注入 |
...
- **推理参数注入** — 新增 `REASONING_EFFORT` 环境变量，自动注入 `reasoning_effort` 参数（兼容 Claude、OpenAI、Google Gemini OpenAI兼容端点）
